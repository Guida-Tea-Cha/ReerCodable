...
Pod::Spec.new do |s|
  ...
  s.pod_target_xcconfig = {
    'OTHER_SWIFT_FLAGS' => '-Xfrontend -load-plugin-executable -Xfrontend ${BUILD_DIR}/ReerCodable/release/ReerCodableMacros-tool#ReerCodableMacros'
  }
  ...
  script = <<-SCRIPT
    env -i PATH="$PATH" "$SHELL" -l -c "swift build -c release --package-path \\"$PODS_TARGET_SRCROOT\\" --build-path \\"${BUILD_DIR}/ReerCodable\\""
  SCRIPT
  ...
  s.script_phase = {
    ...
    :output_files => [
      '${BUILD_DIR}/ReerCodable/release/ReerCodableMacros-tool'
    ]
  }
